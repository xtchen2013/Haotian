...
class ERA5ES(object):
    ...
    def _get_files_stats(self, enable_logging):
        ...
        # print("number of data shards "+str(self.num_shards))
        # print("numer of batch size" + str(self.batch_size))
    ...
    def __call__(self, sample_info):
        ...
        step = self.dt # time step only for single step
        ...
        if self.add_zenith:
            ...
            ############################################
            tmp_zen_inp, tmp_zen_tar = self._compute_zenith_angle(local_idx, year_idx)
            # print(zen_tar.shape)
            # print(tmp_zen_tar.shape)
            ####################################
            # adjust to match input dimensions #
            ####################################
            zen_inp[...] = tmp_zen_inp[...,:self.img_shape_x,:self.img_shape_y]
            zen_tar[...] = tmp_zen_tar[...,:self.img_shape_x,:self.img_shape_y]
