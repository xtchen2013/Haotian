...
- SwinTransformerV2CrBlock: Implements a block of the Swin Transformer with options 
                            for relative positional biases and shifted windows for attention.
- SwinTransformerV2CrStage: Defines a stage in the Swin Transformer,
                            potentially with multiple layers of SwinTransformerV2CrBlock.
- SwinTransformerV2Cr: The main class representing the Swin Transformer V2 model,
                       handling the overall architecture setup, including stacking of stages and producing final outputs.
- WindowMultiHeadAttention and
- WindowMultiHeadAttentionNoPos: Implement multi-head self-attention mechanisms over windows of the input feature map,
                                 with optional relative positional encoding.

Functions:
- bchw_to_bhwc,
- bhwc_to_bchw: Utility functions to transpose tensors between the batch-channel-height-width (BCHW) format 
                and the batch-height-width-channel (BHWC) format.
- window_partition,
- window_reverse: Functions to partition a feature map into windows and to reverse this partitioning, respectively.
"""
...
def swinv2net(params, checkpoint_stages=False):
    ...
    return SwinTransformerV2Cr(img_size=params.img_size,                     # (720, 1440)
                               patch_size=params.patch_size,                 # 4
                               depths=(params.depth, ),                      # (2, )
                               num_heads=(params.num_heads, ),               # (8, )
                               in_chans=params.n_in_channels,                # 71
                               out_chans=params.n_out_channels,              # 71
                               embed_dim=params.embed_dim,                   # 768
                               img_window_ratio=params.window_ratio,         # 80
                               drop_path_rate=params.drop_path_rate,         # 0.1
                               full_pos_embed=params.full_pos_embed,         # True
                               rel_pos=params.rel_pos,                       # False
                               mlp_ratio=params.mlp_ratio,                   # 4
                               checkpoint_stages=act_ckpt,                   # True
                               residual=params.residual,                     # True
                               )
    # swinv2net = SwinTransformerV2Cr(img_size=(720, 1440),
    #                                 patch_size=4,
    #                                 depths=(2, ),
    #                                 num_heads=(8,),
    #                                 in_chans=71,
    ...
    #                                 residual=True,
    #                                 )
    # summary(swinv2net, input_size=(1, 71, 720, 1440))
...
class WindowMultiHeadAttentionNoPos(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        # x = torch.randn(400, 162, 768).to(device='cuda')
        Bw, L, C = x.shape
...
class SwinTransformerV2CrBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = window_attn_block(dim=dim,
                                      num_heads=num_heads,
        ...
        # summary(attn.to(device='cuda'), input_size=(400, 162, 768))
        # summary(blocks.to(device='cuda'), input_size=(1, 180, 360, 768))
    ...
    def _shifted_window_attn(self, x):
        # cyclic shift
        # x = torch.randn(1, 180, 360, 768).to(device='cuda')
        B, H, W, C = x.shape
        sh, sw = self.shift_size  # shift height, shift width
        do_shift: bool = any(self.shift_size)  # return shift_size not zero
        if do_shift:
        ...
        # W-MSA/SW-MSA
        # x_windows = torch.randn(400, 162, 768).to(device='cuda')
        # attn_mask = torch.randn(400, 162, 162).to(device='cuda')
        attn_windows = self.attn(x_windows, mask=self.attn_mask)  # num_windows * B, window_size * window_size, C
        # summary(attn.to(device='cuda'), input_size=(400, 162, 768))


        # reverse cyclic shift
        attn_windows = attn_windows.view(-1, self.window_size[0], self.window_size[1], C) # merge windows
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # x = torch.randn(1, 71, 720, 1440).to(device='cuda')
        B, C, H, W = x.shape
        _assert(H == self.img_size[0], f"Input image height ({H}) doesn't match model ({self.img_size[0]}).")
        _assert(W == self.img_size[1], f"Input image width ({W}) doesn't match model ({self.img_size[1]}).")
        x = self.proj(x)
        # LayerNorm on embed_dim (H, W, C)
        # then back to (C, H, W)
...
class SwinTransformerV2CrStage(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.Sequential(*[SwinTransformerV2CrBlock(dim=embed_dim,
                                                               num_heads=num_heads,
        ...
        # from networks.swinv2_global import SwinTransformerV2CrBlock
        # summary(blocks.to(device='cuda'), input_size=(1, 180, 360, 768))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            output (torch.Tensor): Output tensor of the shape [B, 2 * C, H // 2, W // 2]
        """
        # x = torch.randn(1, 768, 180, 360).to(device='cuda')
        x = bchw_to_bhwc(x) # (1, 180, 360, 768)
        x = self.downsample(x)
        for block in self.blocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                x = block(x)
        x = bhwc_to_bchw(x) # (1, 768, 180, 360)
        return x
...
class SwinTransformerV2Cr(nn.Module):
    ...
    def __init__(self,
        ...
        # summary(patch_embed.to(device='cuda'), input_size=(1, 71, 720, 1440), device='cuda')
        ...
        for stage_idx, (depth, num_heads) in enumerate(zip(depths, num_heads)):
            stages += [SwinTransformerV2CrStage(embed_dim=in_dim,
                                                depth=depth,
                                                downscale=False,   # stage_idx != 0
                                                feat_size=(patch_grid_size[0] // in_scale, patch_grid_size[1] // in_scale),
                                                num_heads=num_heads,
                                                window_size=window_size,
                                                mlp_ratio=mlp_ratio,
                                                init_values=init_values,
                                                ...
                                                extra_norm_stage=extra_norm_stage or (stage_idx + 1) == len(depths),
                                                sequential_attn=sequential_attn,
                                                norm_layer=norm_layer,
                                                rel_pos=rel_pos,
        ...
        # summary(stages.to(device='cuda'), input_size=(1, 768, 180, 360), device='cuda')
    ...
    def forward_head(self, x: torch.Tensor) -> torch.Tensor:
        # x = torch.randn(1, 768, 180, 360).to(device='cuda')
        # x = torch.randn(1, 768*2, 180//2, 360//2).to(device='cuda')
        B, _, h, w = x.shape
        x = bchw_to_bhwc(x)
        x = self.head(x)
    ...
    def update_input_size(self,
        ...
        for index, stage in enumerate(self.stages):
            ...
            stage.update_input_size(new_window_size=new_window_size,
                                    new_img_size=(new_patch_grid_size[0] // stage_scale, new_patch_grid_size[1] // stage_scale),
                                    )

    @torch.jit.ignore  # stem and embed
    def group_matcher(self, coarse=False):
        return dict(stem=r'^patch_embed',
                    blocks=r'^stages\.(\d+)' if coarse else
                    [(r'^stages\.(\d+).downsample', (0,)), (r'^stages\.(\d+)\.\w+\.(\d+)', None), ],
                    )
